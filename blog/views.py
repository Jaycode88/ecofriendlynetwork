...
def blog_list(request):
    """
    Render a page that lists all blog posts, including the user's
    favorite posts if authenticated.
    ...
    if request.user.is_authenticated:
        try:
            # Retrieve user's favorite posts & store IDs in favorite_post_ids
            favorite_posts = Favorite.objects.get(
                user=request.user).posts.all()
    ...
    return render(
        request, 'blog/blog_list.html', {
            'posts': posts, 'favorite_post_ids': favorite_post_ids})


def blog_detail(request, pk):
    """
    Render a page that displays the details of a specific blog post,
    including favorite status.
    ...
    # Get blog post with  given pk or return a 404
    ...
    return render(
        request, 'blog/blog_detail.html', {
            'post': post, 'is_favorite': is_favorite})
...
def favorite_posts_list(request):
    ...
    if request.user.is_authenticated:
        try:
            favorite = Favorite.objects.get(user=request.user)
            # Retrieve user's favorite posts & store in 'posts' list
            posts = favorite.posts.all()
            favorite_post_ids = [post.id for post in posts]
        except Favorite.DoesNotExist:
            posts = []
            favorite_post_ids = []
    # Render favorite_posts_list template with user's favorite posts
    return render(
        request, 'blog/favorite_posts_list.html', {
            'posts': posts, 'favorite_post_ids': favorite_post_ids})
