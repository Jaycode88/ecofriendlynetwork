...
class TestBlogViews(TestCase):
    ...
    def test_edit_post_view_with_superuser(self):
        # Create and login as a superuser
        superuser = User.objects.create_superuser(
                'editsuperuser', 'editsuperuser@test.com', 'editpassword')
        ...
        response = self.client.get(
            reverse('edit_post', args=[post_to_edit.pk]))
        self.assertEqual(response.status_code, 200)
        ...
        self.assertRedirects(
            response, reverse('blog_detail', args=[post_to_edit.pk]))

    def test_delete_post_view_with_superuser(self):
        # Create and login as a superuser
        superuser = User.objects.create_superuser(
            'deletesuperuser', 'deletesuperuser@test.com', 'deletepassword')
        self.client.login(
            username='deletesuperuser', password='deletepassword')
        ...
        response = self.client.post(
            reverse('delete_post', args=[post_to_delete.pk]))
        self.assertEqual(
            Post.objects.count(), 1)  # The post created in setUp still exists
        self.assertRedirects(response, reverse('blog_list'))
    ...
    def test_add_to_favorite_posts(self):
        ...
        response = self.client.post(
            reverse('add_to_favorite_posts', args=[self.post.pk]))
        self.assertTrue(
            self.post in Favorite.objects.get(user=self.user).posts.all())
        self.assertRedirects(
            response, reverse('blog_detail', args=[self.post.pk]))
    ...
    def test_remove_from_favorite_posts(self):
        ...
        # test removing it
        response = self.client.post(
            reverse('remove_from_favorite_posts', args=[self.post.pk]))
        self.assertFalse(
            self.post in Favorite.objects.get(user=self.user).posts.all())
        self.assertRedirects(response, reverse('blog_list'))
